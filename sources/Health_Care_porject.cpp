...
public:
    void takeSurvey() {
        ...
        if (exerciseFreq == 1 || exerciseFreq == 2) {
            cout << "성인을 기준으로 일주일에 최소 5번, 하루에 30분 이상의 저강도 또는 중강도 운동을 권장합니다." << endl;
            cout << "일주일에 최소 3일, 하루에 20분 이상의 고강도 운동을 권장합니다." << endl;
        }
        ...
        if (exerciseFreq == 1) {
            ...
            if (exerciseType == 1) {
                ...
                cout << "스쿼트, 데드리프트, 벤치 프레스, 체인지 푸시업 등의 운동을 권장합니다." << endl;
            }
        }

        cout << "2. 평소 잠을 몇시간 정도 자나요? (1: 5시간 미만, 2: 6시간~9시간, 3: 10시간 이상): ";
        cin >> sleepHours;

        if (sleepHours == 1) {
            cout << "성인을 기준으로 최소 6시간의 수면이 좋습니다." << endl;
            cout << "수면이 오지 않는다면 따뜻한 샤워, 명상, 독서, 느긋한 산책, 차 마시기 등을 추천합니다." << endl;
        }
        else if (sleepHours == 3) {
            cout << "하루에 10시간 이상 자는 것이 좋지 않습니다." << endl;
            cout << "잠을 많이 잘 경우 스트레스, 뇌졸중, 약물 부작용 등의 원인이 될 수 있습니다." << endl;
        }
        ...
        if (breakfastFreq == 1) {
            cout << "아침식사는 집중력/기억력 증진, 에너지 공급, 체중 감량, 면역력 증진, 혈당 감소 등에 도움이 됩니다." << endl;
            cout << "통밀 토스트, 단백질 쉐이크, 그릭 요거트, 오트밀 등을 간단한 아침식사로 추천합니다." << endl;
        }
    }
};
...
public:
    void takeSurvey() {
        ...
        if (diseaseChoice == 1) {
            ...
            vector<string>bodyParts = { "뇌", "피부","목", "복부(1.위, 2.간, 3.장)","생식기","비뇨기","뼈" };
